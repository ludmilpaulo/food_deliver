...
export default function PayoutsAdmin() {
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-3">{t("payouts", "Payouts")}</h2>
      <div className="space-y-3">
        {list.map((p) => (
          <div key={p.id} className="border rounded-xl p-3 bg-white">
            <div className="flex items-center justify-between">
              <div className="font-semibold">{t("partner", "Partner")} {p.parceiro}</div>
            ...
            <div className="mt-2 flex gap-2">
              <button className="px-3 py-1 rounded bg-blue-600 text-white" onClick={() => onProcessing(p.id)}>
                {t("processing", "Processing")}
              </button>
              <button className="px-3 py-1 rounded bg-green-600 text-white" onClick={() => onComplete(p.id)}>
                {t("complete", "Complete")}
              </button>
            </div>
          </div>
